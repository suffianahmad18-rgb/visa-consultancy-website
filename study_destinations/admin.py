...
from .models import (
    StudyDestination, DestinationSection, TuitionTable, 
    IntakeTable, Scholarship, VisaRequirement, PostStudyWork
)
...
class TuitionTableInline(admin.TabularInline):
    model = TuitionTable
    extra = 1
    fields = ['program_name', 'program_level', 'tuition_fee_min', 'tuition_fee_max', 'duration_years', 'order']

class IntakeTableInline(admin.TabularInline):
    model = IntakeTable
    extra = 1
    fields = ['intake_name', 'intake_month', 'application_deadline', 'is_main_intake', 'order']
...
class StudyDestinationAdmin(admin.ModelAdmin):
    list_display = ['country_name', 'country_code', 'is_published', 'is_featured', 'order', 'created_at']
    ...
    fieldsets = (
        ...
        ('Quick Facts', {
            'fields': ('quick_fact_1', 'quick_fact_2', 'quick_fact_3', 'quick_fact_4')
        }),
        ('SEO & Settings', {
            'fields': ('meta_title', 'meta_description', 'is_published', 'is_featured', 'order')
        }),
        ('Preview', {
            'fields': ('preview_link',),
            'classes': ('collapse',)
    ...
    def preview_link(self, obj):
        if obj.pk:
            return format_html('<a href="{}" target="_blank">View Live Page</a>', obj.get_absolute_url())
...
class DestinationSectionAdmin(admin.ModelAdmin):
    list_display = ['destination', 'section_title', 'section_type', 'order', 'is_active']
...
class TuitionTableAdmin(admin.ModelAdmin):
    list_display = ['destination', 'program_name', 'program_level', 'tuition_fee_min', 'duration_years']
...
class IntakeTableAdmin(admin.ModelAdmin):
    list_display = ['destination', 'intake_name', 'intake_month', 'application_deadline', 'is_main_intake']
...
class ScholarshipAdmin(admin.ModelAdmin):
    list_display = ['destination', 'scholarship_title', 'scholarship_type', 'amount', 'is_active']
...
class VisaRequirementAdmin(admin.ModelAdmin):
    list_display = ['destination', 'visa_name', 'visa_type', 'processing_time', 'visa_fee']
