...
class AdminInboxView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        return Message.objects.filter(receiver__is_staff=True).order_by("-created_at")

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["unread_count"] = Message.objects.filter(receiver=self.request.user, is_read=False).count()
        return context
